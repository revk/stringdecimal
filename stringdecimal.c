...
{				// Unsigned div (i.e. final sign already set in r) and set in r if needed
  ...
  for (int p = mag; p > mag - sig; p--)
    {
      int n = 0;
      shift = a->mag  - (mag - p) - b->mag;
#ifdef DEBUG
       char *V = output (v);
       fprintf(stderr,"v=%s p=%d shift=%d\n",V,p,shift);
       free(V);
#endif
      while (n < 9 && ucmp (v, base[n], shift) > 0)
	n++;
      if (n)
	{
	  sd_t *s = usub (NULL, v, base[n - 1], shift);
	  free (clean (v));
	  v = s;
	}
      r->d[mag - p] = n;
...
{				// Parse a sum and process it using basic maths
  ...
  int operators = 0, operatormax = 0;
  int operands = 0, operandmax = 0;
  struct operator_s
  {
    int level;
    char operator;
